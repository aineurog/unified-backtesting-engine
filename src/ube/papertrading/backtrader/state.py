"""Backtrader paper-trading state (§4/§5 of the backtrader paper plan).

The backtrader backend is *stateless and recomputable*: every ``step`` re-runs
``backtrader_adapter`` over a window of bars and folds the resulting trades into the
...
:class:`~ube.papertrading.state.PaperState`) and :class:`BacktraderPaperState`, whose methods
...
def fold_cash_commission(
    ...
    Funding is deliberately excluded (the adapter's sizing fold excludes it too — see the
    module docstring). ``bound_ns=None`` folds every event.
...
def window_start_ns(
    ...
    * Open position — the carried entry bar (no warmup, no cursor max): the window must
      contain the entry so backtrader re-opens and manages the carried trade.
...
class BacktraderPaperState(PaperState):
    ...
    # -- persistence: write / verify the backtrader engine tag (aux_data) ----

    def save(self, path: str, run_id: str = "default") -> None:
        """Persist, stamping ``aux_data['backtrader']`` so the row is recognizable on reload."""
        aux: dict[str, Any] = dict(self.aux_data) if self.aux_data else {}
        aux["backtrader"] = dict(BACKTRADER_ENGINE_TAG)
        self.aux_data = aux
        super().save(path, run_id=run_id)

    @classmethod
    def load(cls, path: str, run_id: str = "default") -> BacktraderPaperState:
        """Load and verify the row was written by the backtrader engine.

        Raises:
            StateCorruptionError: The row lacks the ``aux_data['backtrader']`` engine marker —
                it was written by a different (e.g. vectorbt) engine and must not be
                resumed with the backtrader backend.
        """
        obj = cast("BacktraderPaperState", super().load(path, run_id=run_id))
        tag = (obj.aux_data or {}).get("backtrader")
        if not isinstance(tag, dict) or tag.get("engine") != "backtrader":
            raise StateCorruptionError(
                f"paper_state run_id={run_id!r} was not written by the backtrader engine "
                "(missing aux_data['backtrader'] engine tag); refusing to resume across engines"
            )
        return obj