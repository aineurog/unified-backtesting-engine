"""backtrader engine overrides — the typed ``engine_overrides`` namespace (§4.3, §7.2).

backtrader-specific knobs live here, never in :class:`~ube.core.config.BacktestConfig` (§4.2):
the base config only carries them as an opaque ``Mapping``, and :func:`validate_overrides`
enforces the per-adapter schema at ``BacktraderAdapter.run`` time. Unknown keys and wrong types
raise :class:`~ube.core.errors.ConfigError` naming the offending field (requirements §7.2).

Overrides that backtrader actually needs mirror the vectorbt namespace: the account starting
balance, the funding cadence (which otherwise comes from the canonical instrument — §4.5, so
the override is a precedence fallback), the asset-class lot grid (``size_precision`` /
``size_increment``), the account type (``cash`` vs ``margin`` — a cash account ignores the
sizing ``leverage``, mirroring the nautilus adapter), and the FX synthetic-rates family
(``synthetic_rates`` / ``fixed_conversions`` / ``currency_fx`` / ``fx_rates``) used to
normalize multi-currency results into ``base_currency`` (§4.6).
"""
...
#: The synthetic-rates alias family (mirrors the nautilus/vectorbt override namespace).
...
class BacktraderEngineOverrides(TypedDict, total=False):
    """Engine-specific overrides for the backtrader adapter (dev-guide §4.3, requirements §4.2).

    Every key is optional; backtrader defaults are applied for anything not provided. Values are
    validated by :func:`validate_overrides` at ``BacktraderAdapter.run`` time.
...
def parse_synthetic_rates(value: Any, field: str = "fx_rates") -> dict[str, float]:
    ...
    rates are never guessed (§4.7). This is the backtrader-side mirror of the nautilus
...
def validate_overrides(overrides: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    for field, value in overrides.items():
        ...
        if validator is None:
            ...
            raise ConfigError(
                f"unknown engine override {field!r} for the backtrader adapter "
                f"(valid keys: {valid})"
            )
