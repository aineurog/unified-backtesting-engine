"""Canonical :class:`~ube.core.instrument.Instrument` -> backtrader parameter mapping.

backtrader does not require a native instrument object (it operates on a price feed and a
``CommInfo``), but the engine still needs the per-asset-class *parameters* that drive correct
sizing and accounting: the size precision / lot increment, the contract multiplier, the
settlement currency, and the funding cadence. This module is the backtrader analogue of
:mod:`ube.adapters.vectorbt_adapter.instrument_map` (requirements §4.5) — a single, explicit
...
#: Asset classes that trade on margin (the sizing ``leverage`` is honored). Cash-like classes
#: (stocks, futures-index, commodities spot) trade at full notional.
_MARGIN_ASSET_CLASSES: frozenset[str] = frozenset({"crypto_perp", "forex"})
...
def round_to_increment(qty: float, increment: float) -> float:
    ...
    A non-positive increment is a no-op (fractional sizing is allowed).
    """
...
def build_instrument(
    ...
    """Resolve the backtrader parameters for ``canonical`` (§4.5).
    ...
    Returns:
        The resolved :class:`BtInstrument`.

    Raises:
        InvalidInstrumentError: ``asset_class`` is not supported by the backtrader adapter.
    """
    asset_class = canonical.asset_class
    if asset_class not in _DEFAULT_SIZE_PRECISION:
        raise InvalidInstrumentError(
            f"asset_class={asset_class!r} is not supported by the backtrader adapter; "
    ...
    # (mirroring the nautilus/vectorbt adapters), so the schedule is not silently rewritten.
    # An explicit engine override still wins; otherwise the canonical instrument's
    ...
    return BtInstrument(
        ...
        margin=asset_class in _MARGIN_ASSET_CLASSES,
    )