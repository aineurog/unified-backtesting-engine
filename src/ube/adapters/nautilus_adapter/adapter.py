...
_KIND_RANK: dict[EventType, int] = {
    ...
    EventType.ORDER_SUBMITTED: 1,
    EventType.FILL: 2,
    EventType.COMMISSION: 3,
    EventType.POSITION_CHANGE: 4,
    EventType.FUNDING_PAYMENT: 5,
}
...
class NautilusAdapter(EngineAdapter):
    ...
    def run(
        ...
        *,
        ...
        Args:
            ...
            aux_data: Named derived series (§5.2), keyed by name (e.g.
                ``{"atr_12h": series}``). A single named ATR series is resolved and passed
                to the actor's exit book; an absent name falls back to the exit's own
                ``period``-computed ATR.
    ...
    def _fold(
        ...
        *,
        ...
        # Orders the actor submitted (§4.6): inert records — no monetary effect, so
        # they do not touch trades/equity — sorted before fills by ``_KIND_RANK``.
        for bar_idx, order_id, side, quantity in actor.submitted_orders:
            _add(
                LedgerEvent(
